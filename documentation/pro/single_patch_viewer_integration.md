...
| `predict(point_spec, *, encode_prompt=False, points_space_override=None, export=True, save_debug_patch=None, save_debug_patch_prompts=False, case_id=None, wait_timeout=None)` | Uses cached preprocess; optional `case_id` must match the cached paths tuple or `ValueError`. Returns exported path `{ofile}.{file_ending}` or `None` if `export=False`/`ofile` missing. |
...
on_click(point_dict):
    path = session.predict(point_dict, encode_prompt=user_wants_prompt)
    update_overlay(path)
