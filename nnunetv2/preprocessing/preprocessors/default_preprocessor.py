...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        # Note: 2D configurations are no longer supported, but keeping this check for safety/compatibility
        # with existing plans files that may have 2D configs
    ...
    def _run_internal(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        display.update_step("Loading plans", 50)
        ...
        display.complete_step("Loading plans")

        if self.verbose:
            display.show_info(f'Preprocessing the following configuration: {configuration_name}')
            display.show_info(str(configuration_manager))
