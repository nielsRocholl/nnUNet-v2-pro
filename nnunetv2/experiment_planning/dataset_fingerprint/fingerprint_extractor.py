...
class DatasetFingerprintExtractor(object):
    ...
    def run(self, overwrite_existing: bool = False, display: Optional[object] = None) -> dict:
        if display is None:
            from nnunetv2.utilities.cli_display import FingerprintDisplay
            display = FingerprintDisplay(self.dataset_name, len(self.dataset), self.verbose)
            with display:
                return self._run_internal(overwrite_existing, display)
        else:
            return self._run_internal(overwrite_existing, display)
    
    def _run_internal(self, overwrite_existing: bool, display: object) -> dict:
            ...
            if not isfile(properties_file) or overwrite_existing:
                ...
                with multiprocessing.get_context("spawn").Pool(self.num_processes) as p:
                    ...
                    while len(remaining) > 0:
                        ...
                        display.update_step("Analyzing cases", int(100 * completed_cases / len(self.dataset)))
                        remaining = [i for i in remaining if i not in done]
                        sleep(0.1)

                display.complete_step("Analyzing cases")
                display.update_step("Aggregating results", 50)

                results = [i.get()[0] for i in r]

                display.complete_step("Aggregating results")
                display.update_step("Computing statistics", 50)
                ...
                display.complete_step("Computing statistics")
                display.update_step("Saving fingerprint", 50)
                ...
                display.complete_step("Saving fingerprint")
                display.show_summary(fingerprint)
            else:
                fingerprint = load_json(properties_file)
                display.show_info("Fingerprint already exists, skipping extraction")
                for step in display.steps:
                    display.complete_step(step)
