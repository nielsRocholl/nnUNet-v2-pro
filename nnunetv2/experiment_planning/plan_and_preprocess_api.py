...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
    ...
    if experiment_planner_class_name == 'ExperimentPlanner':
        ...
        console.print("[yellow]INFO: You are using the old nnU-Net default planner. We have updated our recommendations. "
              "Please consider using those instead! "
              "Read more here: https://github.com/MIC-DKFZ/nnUNet/blob/master/documentation/resenc_presets.md[/yellow]")
    ...
    for n, c in zip(num_processes, configurations):
        if c not in plans_manager.available_configurations:
            if current_display is not None:
                current_display.add_message(
                    f"Configuration {c} not found in plans file {plans_identifier + '.json'}. Skipping.",
                    "warning"
                )
            else:
                console = Console()
                console.print(f"[yellow]INFO: Configuration {c} not found in plans file {plans_identifier + '.json'} of "
                             f"dataset {dataset_name}. Skipping.[/yellow]")
            continue
        
        if current_display is None:
            current_display = PreprocessingDisplay(dataset_name, c, len(dataset), verbose)
