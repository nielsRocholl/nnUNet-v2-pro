...
from nnunetv2.utilities.cli_display import PlanningDisplay
...
class ExperimentPlanner(object):
    ...
    def get_plans_for_configuration(self,
        ...
        assert len(spacing) == 3, f"Only 3D configurations are supported. Spacing must have 3 dimensions, got {len(spacing)}"
        num_input_channels = len(self.dataset_json['channel_names'].keys()
                                 if 'channel_names' in self.dataset_json.keys()
                                 else self.dataset_json['modality'].keys())
        max_num_features = self.UNet_max_features_3d
        ...
        reference = self.UNet_reference_val_3d * \
    ...
    def plan_experiment(self, verbose: bool = False):
        """
        ...
        hard.
        """
        with PlanningDisplay(self.dataset_name, verbose=verbose) as display:
            ...
            display.complete_step("Determining transpose")
            ...
            display.complete_step("Computing target spacing")
            ...
            if new_median_shape_transposed[0] != 1:
                ...
                display.complete_step("Planning 3D fullres configuration")
                display.show_configuration("3D Full Resolution", plan_3d_fullres)
                ...
                while num_voxels_in_patch / median_num_voxels < self.lowres_creation_threshold:
                    ...
                    if verbose:
                        display.show_lowres_iteration(
                            iteration,
                            list(lowres_spacing),
                            plan_3d_lowres['patch_size'],
                            list(plan_3d_fullres['spacing'] / lowres_spacing * new_median_shape_transposed)
                        )
                ...
                if np.prod(new_median_shape_transposed, dtype=np.float64) / median_num_voxels < 2:
                    ...
                    display.complete_step("Planning 3D lowres configuration")
                    display.show_configuration("3D Low Resolution", plan_3d_lowres)
            ...
            display.complete_step("Saving plans")
            return plans
