...
class nnUNetTrainer(object):
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        assert len(patch_size) == 3, f"Only 3D patch sizes supported, got {len(patch_size)}"
    ...
    def print_to_log_file(self, *args, also_print_to_console=True, add_timestamp=True):
        # If display is active, suppress console output (display handles it)
    ...
    def on_train_start(self):
        ...
        # Show configuration in display if available
        if self.display is not None:
            config_dict = {
    ...
    def on_train_epoch_start(self):
        ...
        # Update display
        if self.display is not None:
            self.display.update_epoch(self.current_epoch, self.num_epochs)
    ...
    def on_epoch_end(self):
        ...
        dice = np.round(self.logger.my_fantastic_logging['mean_fg_dice'][-1], decimals=4)
        lr = np.round(self.optimizer.param_groups[0]['lr'], decimals=5)
        ...
        if self._best_ema is None or self.logger.my_fantastic_logging['ema_fg_dice'][-1] > self._best_ema:
            ...
            # Update display
            if self.display is not None:
                self.display.show_best_model(self.current_epoch, self._best_ema)
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                result = self.train_step(next(self.dataloader_train))
                train_outputs.append(result)
                # Update display with batch progress
                if self.display is not None:
                    loss = float(result['loss']) if isinstance(result['loss'], np.ndarray) else result['loss']
                    self.display.update_training_batch(batch_id + 1, self.num_iterations_per_epoch, loss)
            ...
            with torch.no_grad():
                ...
                for batch_id in range(self.num_val_iterations_per_epoch):
                    ...
                    # Update display with validation batch progress
                    if self.display is not None:
                        self.display.update_validation_batch(batch_id + 1, self.num_val_iterations_per_epoch)
